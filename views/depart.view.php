...
<head>
    ...
    <title>📤 Courriers sortants - OpenGestiCourrier</title>
...
<body>
    ...
    <div id="successModal" class="modal">
        <div class="modal-content" style="text-align: center;">
            <h2 style="color: #10b981;"><i class="fas fa-check-circle"></i> Enregistrement réussi !</h2>
            <p>Le courrier sortant a été enregistré avec succès.</p>
        </div>
    </div>
    ...
    <div id="searchModal" class="modal">
        <div class="modal-content">
            ...
            <h2 style="color: #059669;"><i class="fas fa-search"></i> Rechercher un courrier sortant</h2>
    ...
    <div class="main-container">
        ...
        <div class="page-action-bar">
            <div class="page-title-badge">
                <i class="fas fa-paper-plane"></i> Enregistrement des courriers sortants
            </div>
        ...
        <div class="form-container" id="form-depart">
            <form id="courrier-form" action="save_courrier.php" method="post" enctype="multipart/form-data">
                ...
                <div class="form-section-card">
                    ...
                    <div class="form-grid-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="flux" name="flux" value="DEPART" readonly>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="date">Date d'expédition :</label>
                ...
                <!-- Carte 2 : Identification & Destinataire -->
                <div class="form-section-card">
                    <div class="form-section-header">
                        <h2><i class="fas fa-user-check"></i> 2. Destinataire & Identification du courrier</h2>
                    </div>
                    <div class="form-grid-2">
                        <div class="form-group">
                            <label for="expediteur">📌 Destinataire :</label>
                            <input type="text" id="expediteur" name="expediteur" placeholder="Tapez pour rechercher un destinataire..." required>
                            <input type="hidden" id="expediteur_id" name="expediteur_id">
                            <input type="text" name="new_expediteur" id="new_expediteur" placeholder="+ Ajouter un nouveau destinataire si inexistant" style="margin-top:6px;">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="courrier_arrive">📌 Courrier arrivé associé :</label>
                            <input type="text" id="courrier_arrive" name="courrier_arrive" placeholder="Sélectionnez un courrier arrivé associé...">
                            <input type="hidden" id="courrier_arrive_id" name="courrier_arrive_id">
                        </div>
                        <div class="form-group form-grid-full">
                ...
                <div class="form-section-card">
                    ...
                    <div id="live-stamp-preview-container" class="live-stamp-preview-container" style="display: none;">
                        <div class="preview-toolbar-bar">
                            <div id="doc-preview-info-badge" style="font-weight: 700; color: #047857; font-size: 0.92rem;">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                                ...
                                <div id="pdf-page-nav-controls" style="display: none; align-items: center; gap: 6px; background: #ffffff; padding: 3px 8px; border-radius: 6px; border: 1px solid #cbd5e1;">
                                    ...
                                    <span style="font-size: 0.85rem; font-weight: 700; color: #047857; padding: 0 4px;">Page <span id="pdf-page-num">1</span> / <span id="pdf-page-count">1</span></span>
                                ...
                                <span style="font-size: 0.82rem; color: #475569; font-weight: 600; background: #e2e8f0; padding: 4px 10px; border-radius: 6px;"><i class="fas fa-hand-pointer" style="color: #10b981;"></i> Déplacez & redimensionnez le tampon au coin</span>
                                ...
                                <span id="zoom-level-indicator" style="font-weight: 700; color: #047857; font-size: 0.88rem; background: #ffffff; padding: 4px 10px; border-radius: 6px; border: 1px solid #cbd5e1;">100%</span>
                            </div>
                        </div>
                ...
                <div style="margin-top: 24px;">
                    <button type="submit" class="btn-submit-main">
                        <i class="fas fa-save"></i> Enregistrer le Courrier Sortant
                    </button>
                </div>
            </form>
        </div>

        <!-- Section Liste des courriers sortants -->
        <div class="table-header-bar">
            <h3><i class="fas fa-list-alt"></i> Liste récente des courriers sortants</h3>
        ...
        <div class="table-container2" style="margin-top: 0; border-radius: 0 0 12px 12px;">
            <table id="courriers-depart-table" style="font-size: 13px;">
                <thead>
                    <tr>
                        <th>N° Ordre</th>
                        <th>Date</th>
                        ...
                        <th>Catégorie</th>
                        <th>Documents</th>
                        <th style="color:#f59e0b;">N° Ordre<br>Arrivé</th>
                        <th style="color:#f59e0b;">Courrier Arrivé</th>
                    </tr>
                </thead>
                <tbody>
                    <!-- JS population -->
    ...
    <?php include 'partials/depart_script.html'; ?>
    ...
    function loadCourrierByNumOrdre() {
        ...
        fetch(`obtenir_courrier_depart.php?num_ordre=${numOrdre}&annee=${annee}`)
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                    alert(data.error);
                    ...
                    document.getElementById('courrier_arrive').value = data.courrier_arrive_num_ordre || '';
                    document.getElementById('courrier_arrive_id').value = data.courrier_arrive_id || '';
    ...
    function removeFile(elementId, inputName, inputId, docNum) {
        if (confirm("Êtes-vous sûr de vouloir supprimer ce document joint ?")) {
            ...
            const flux = document.getElementById('flux').value || 'DEPART';
