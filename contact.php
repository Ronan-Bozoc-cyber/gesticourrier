...
<head>
    ...
    <title>📇 Contacts - OpenGestiCourrier</title>
...
<div class="main-container">
    ...
    <div class="page-action-bar">
        <div class="page-title-badge" style="color: #2563eb;">
            <i class="fas fa-address-book"></i> Enregistrement des contacts
        </div>
        <button onclick="document.getElementById('search-expediteur').focus()" class="btn-action-search" style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%);">
            <i class="fas fa-search"></i> Filtrer / Rechercher un contact
        </button>
    </div>
    ...
    <?php if (!$can_edit): ?>
        ...
        <style>
            #add-expediteur-form input, 
            #add-expediteur-form textarea,
            .btn-submit-main,
            .edit-btn {
    ...
    <!-- Formulaire principal d'enregistrement direct (exactement comme arrive.php / depart.php) -->
        ...
        <form id="add-expediteur-form">
            <div class="form-section-card" style="border-top: 4px solid #2563eb;">
                <div class="form-section-header">
                    <h2 style="color: #2563eb;"><i class="fas fa-user-plus"></i> 1. Identification & Coordonnées du contact</h2>
                </div>
                <div class="form-group">
                    <label for="add-name">📌 Nom du contact / Organisme :</label>
                    <input type="text" id="add-name" name="name" placeholder="Saisir le nom complet ou l'organisme..." required>
                </div>
                <div class="form-group">
                    <label for="add-adresse">📌 Adresse postale complète :</label>
                    <textarea id="add-adresse" name="adresse" rows="3" placeholder="Saisir l'adresse postale complète..." required></textarea>
                </div>
            </div>
    ...
    <!-- Section Liste des contacts (exactement comme arrive.php / depart.php) -->
    <div class="table-header-bar" style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); margin-top: 35px;">
        <h3><i class="fas fa-list-alt"></i> Liste des contacts référencés</h3>
        <div style="display: flex; align-items: center; gap: 10px;">
            <input type="text" id="search-expediteur" placeholder="🔍 Filtrer les contacts..." style="padding: 6px 14px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.4); font-size: 0.88rem; width: 220px; background: rgba(255,255,255,0.9); color: #0f172a;">
        </div>
    </div>
    <div class="table-container2" style="margin-top: 0; border-radius: 0 0 12px 12px;">
        <table id="expediteur-table" style="font-size: 13px;">
            <thead>
                <tr>
                    <th>Nom</th>
                    <th>Adresse</th>
            ...
            <tbody>
                ...
                <tr>
                    <td><strong><?php echo htmlspecialchars($expediteur['name']); ?></strong></td>
                    <td><?php echo htmlspecialchars($expediteur['adresse']); ?></td>
                    <td style="text-align: center;">
                        <button class="edit-btn" data-id="<?php echo $expediteur['id']; ?>" data-name="<?php echo htmlspecialchars($expediteur['name']); ?>" data-adresse="<?php echo htmlspecialchars($expediteur['adresse']); ?>" style="background: #2563eb; color: white; border: none; padding: 6px 14px; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.85rem;"><i class="fas fa-edit"></i> Modifier</button>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <h2 style="color: #2563eb;"><i class="fas fa-user-edit"></i> Modifier le contact</h2>
            <form id="edit-expediteur-form">
                ...
                <div class="form-group">
                    ...
                    <textarea id="edit-adresse" name="adresse" rows="3"></textarea>
                </div>
